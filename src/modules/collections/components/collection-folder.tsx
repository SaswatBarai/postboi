...
import { requestColorMap, SaveRequestToCollectionModal } from './add-request-model';
import { useDeleteRequest, useGetAllRequestsFromCollection } from '@/modules/request/hooks/request';
...
const CollectionFolder = ({collection}:Props) => {
    ...
    const {data:requestData,isPending,isError} = useGetAllRequestsFromCollection(collection.id);
    const {mutateAsync:mutateDeletdAsync} = useDeleteRequest(collection.id)
    const deleteRequest = async (id:string) => {
      await mutateDeletdAsync(id)
    }
        ...
        <div className='flex flex-col w-full'>
            ...
            <CollapsibleContent className='w-full '>
            {
              ...
              isError? (
                <>
                <div className='pl-8 py-2'>
                  <span className="text-xs text-red-400 ">Failed to load requests</span>
                </div>
                </>
              )
              : hasRequest ? (
               ...
               <div className="ml-6 border-l border-zinc-800 pl-4 space-y-1">
                ...
                <>
                <div className='pl-8 py-2'>
                  <span className='text-xs text-zinc-500 italic'>No requests yet</span>
                </div>
                </>
              )
            }
